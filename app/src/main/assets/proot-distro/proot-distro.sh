...
command_install() {
		...
		--override-alias)
			if [ $# -ge 2 ]; then
				...
				if [ -z "$1" ]; then
					...
					if ! grep -Eq '^[a-z0-9][a-z0-9_.+\-]*$' <<< "$1"; then
					msg
	...
	if [ -f "${distro_plugin_script}" ]; then
		...
		if [ -z "${PD_OVERRIDE_TARBALL_URL-}" ] && [ -n "${PD_OVERRIDE_TARBALL_SHA256-}" ]; then
			if ! grep -Eq '^[0-9a-fA-F]{64}$' <<< "${TARBALL_SHA256["$DISTRO_ARCH"]}"; then
			msg
...
command_rename() {
	...
	if ! grep -Eq '^[a-z0-9][a-z0-9_.+\-]*$' <<< "${new_distro_name}"; then
		msg
	...
	if grep -Eq '^.*\.sh$' <<< "${new_distro_name}"; then
		msg
...
command_login() {
	...
	msg "${BLUE}[${GREEN}*${BLUE}] ${CYAN}Detected target architecture: ${YELLOW}${target_arch}${CYAN}.${RST}"
	# log device CPU architecture.
	msg "${BLUE}[${GREEN}*${BLUE}] ${CYAN}Detected device CPU architecture: ${YELLOW}${DEVICE_CPU_ARCH}${CYAN}.${RST}"
	...
	for i in "${!fds[@]}"; do
		...
		realpath "/proc/self/fd/$i" >/dev/null && set -- "--bind=/proc/self/fd/$i:/dev/${fds[i]}" "$@"
	done
	unset i fds
	...
	if $use_termux_home; then
		if [ "$login_user" = "root" ]; then
			...
			if [ -f "${INSTALLED_ROOTFS_DIR}/${distro_name}/etc/passwd" ]; then
				local user_home
				user_home=$(grep -E "^${login_user}:" "${INSTALLED_ROOTFS_DIR}/${distro_name}/etc/passwd" | cut -d: -f 6)
...
command_copy() {
	...
	src_distribution=$(grep -q ':' <<< "${source}" && cut -d':' -f1 <<< "${source}" || true)
	...
	dest_distribution=$(grep -q ':' <<< "${destination}" && cut -d':' -f1 <<< "${destination}" || true)
...
UNAME_M=$(uname -m)
echo "Detected CPU architecture: ${UNAME_M}"
